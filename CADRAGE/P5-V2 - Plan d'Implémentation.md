# P5-V2 : Plan d'Implémentation OpsTracker V2 (Module Réservation)

> **Version** : 2.0.0 **Date** : 2025-01-24 **Auteur** : Framework BA-AI **Statut** : 🟢 PRÊT POUR DÉVELOPPEMENT

---

## 1. Résumé Exécutif

### 1.1 Périmètre V2

La V2 d'OpsTracker ajoute le **module Réservation** permettant aux agents métier et managers de réserver des créneaux d'intervention IT de manière autonome (style "Doctolib").

| EPIC        | Nom                               | US Total  | Priorité |     |
| ----------- | --------------------------------- | --------- | -------- | --- |
| **EPIC-10** | Interface Réservation (End-Users) | 12 US     | Core     |     |
| **EPIC-11** | Gestion Créneaux & Capacité       | 8 US      | Core     |     |
| **EPIC-12** | Notifications & Agenda            | 6 US      | Core     |     |
| **TOTAL**   |                                   | **26 US** |          |     |

### 1.2 Nouvelles Entités

```
Creneau        : Plage horaire réservable pour une campagne
Reservation    : Association Agent ↔ Créneau
Agent          : Utilisateur métier (distinct de Utilisateur IT)
Notification   : Historique des emails envoyés
```

### 1.3 Timeline

|Sprint|Durée|Focus|US|
|---|---|---|---|
|16|1 sem|Setup + Entités|0|
|17|1 sem|Créneaux (EPIC-11 Core)|4|
|18|1.5 sem|Réservation (EPIC-10 Core)|6|
|19|1 sem|Notifications (EPIC-12)|5|
|20|1 sem|Compléments V1|8|
|21|1 sem|Tests + Audit P6|3|
|**TOTAL**|**~7 sem**||**26 US**|

---

## 2. Architecture V2

### 2.1 Modèle de Données (ERD)

```
┌─────────────────────────────────────────────────────────────────────┐
│                        MODÈLE V2 - RÉSERVATION                       │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  ┌──────────────┐         ┌──────────────┐         ┌──────────────┐ │
│  │   Campagne   │────────<│   Creneau    │>────────│  Reservation │ │
│  │   (V1)       │  1:N    │              │   1:N   │              │ │
│  └──────────────┘         │ - date       │         │ - agent_id   │ │
│         │                 │ - heure_debut│         │ - positionne │ │
│         │                 │ - heure_fin  │         │   _par       │ │
│  ┌──────┴───────┐         │ - capacite   │         │ - type       │ │
│  │   Segment    │────────<│ - lieu       │         │   (agent/    │ │
│  │   (V1)       │  N:1    │ - segment_id │         │    manager/  │ │
│  └──────────────┘         │ - verrouille │         │    coord)    │ │
│                           └──────────────┘         │ - created_at │ │
│                                  │                 └──────────────┘ │
│  ┌──────────────┐                │                        │         │
│  │    Agent     │────────────────┴────────────────────────┘         │
│  │              │                                                    │
│  │ - matricule  │         ┌──────────────┐                          │
│  │ - email      │────────<│ Notification │                          │
│  │ - nom/prenom │   1:N   │              │                          │
│  │ - service    │         │ - type       │                          │
│  │ - manager_id │         │ - contenu    │                          │
│  │ - site       │         │ - statut     │                          │
│  └──────────────┘         │ - sent_at    │                          │
│                           └──────────────┘                          │
└─────────────────────────────────────────────────────────────────────┘
```

### 2.2 Entités Doctrine

#### Entity: Creneau

```php
#[ORM\Entity(repositoryClass: CreneauRepository::class)]
#[ORM\Table(name: 'creneau')]
class Creneau
{
    #[ORM\Id]
    #[ORM\GeneratedValue]
    #[ORM\Column]
    private ?int $id = null;

    #[ORM\ManyToOne(targetEntity: Campagne::class)]
    #[ORM\JoinColumn(nullable: false)]
    private ?Campagne $campagne = null;

    #[ORM\ManyToOne(targetEntity: Segment::class)]
    #[ORM\JoinColumn(nullable: true)]
    private ?Segment $segment = null;

    #[ORM\Column(type: Types::DATE_MUTABLE)]
    #[Assert\NotBlank]
    private ?\DateTimeInterface $date = null;

    #[ORM\Column(type: Types::TIME_MUTABLE)]
    #[Assert\NotBlank]
    private ?\DateTimeInterface $heureDebut = null;

    #[ORM\Column(type: Types::TIME_MUTABLE)]
    #[Assert\NotBlank]
    private ?\DateTimeInterface $heureFin = null;

    #[ORM\Column]
    #[Assert\Positive]
    private int $capacite = 1;

    #[ORM\Column(length: 255, nullable: true)]
    private ?string $lieu = null;

    #[ORM\Column]
    private bool $verrouille = false;

    #[ORM\Column(type: Types::DATETIME_MUTABLE)]
    private ?\DateTimeInterface $createdAt = null;

    #[ORM\OneToMany(mappedBy: 'creneau', targetEntity: Reservation::class)]
    private Collection $reservations;

    // Méthodes calculées
    public function getPlacesRestantes(): int
    public function isComplet(): bool
    public function isVerrouille(): bool // verrouille OU date < J-X
}
```

#### Entity: Agent

```php
#[ORM\Entity(repositoryClass: AgentRepository::class)]
#[ORM\Table(name: 'agent')]
class Agent
{
    #[ORM\Id]
    #[ORM\GeneratedValue]
    #[ORM\Column]
    private ?int $id = null;

    #[ORM\Column(length: 50, unique: true)]
    #[Assert\NotBlank]
    private ?string $matricule = null;

    #[ORM\Column(length: 180)]
    #[Assert\NotBlank]
    #[Assert\Email]
    private ?string $email = null;

    #[ORM\Column(length: 100)]
    #[Assert\NotBlank]
    private ?string $nom = null;

    #[ORM\Column(length: 100)]
    #[Assert\NotBlank]
    private ?string $prenom = null;

    #[ORM\Column(length: 100, nullable: true)]
    private ?string $service = null;

    #[ORM\Column(length: 100, nullable: true)]
    private ?string $site = null;

    #[ORM\ManyToOne(targetEntity: self::class)]
    #[ORM\JoinColumn(nullable: true)]
    private ?self $manager = null;

    #[ORM\Column]
    private bool $actif = true;

    #[ORM\OneToMany(mappedBy: 'agent', targetEntity: Reservation::class)]
    private Collection $reservations;
}
```

#### Entity: Reservation

```php
#[ORM\Entity(repositoryClass: ReservationRepository::class)]
#[ORM\Table(name: 'reservation')]
#[ORM\UniqueConstraint(columns: ['agent_id', 'campagne_id'])] // RG-121
class Reservation
{
    #[ORM\Id]
    #[ORM\GeneratedValue]
    #[ORM\Column]
    private ?int $id = null;

    #[ORM\ManyToOne(targetEntity: Agent::class, inversedBy: 'reservations')]
    #[ORM\JoinColumn(nullable: false)]
    private ?Agent $agent = null;

    #[ORM\ManyToOne(targetEntity: Creneau::class, inversedBy: 'reservations')]
    #[ORM\JoinColumn(nullable: false)]
    private ?Creneau $creneau = null;

    #[ORM\ManyToOne(targetEntity: Campagne::class)]
    #[ORM\JoinColumn(nullable: false)]
    private ?Campagne $campagne = null;

    #[ORM\Column(length: 20)]
    private string $typePositionnement = 'agent'; // agent, manager, coordinateur

    #[ORM\ManyToOne(targetEntity: Utilisateur::class)]
    #[ORM\JoinColumn(nullable: true)]
    private ?Utilisateur $positionneePar = null; // RG-125

    #[ORM\Column(type: Types::DATETIME_MUTABLE)]
    private ?\DateTimeInterface $createdAt = null;

    #[ORM\Column(type: Types::DATETIME_MUTABLE, nullable: true)]
    private ?\DateTimeInterface $modifiedAt = null;

    #[ORM\Column(length: 20)]
    private string $statut = 'confirmee'; // confirmee, annulee
}
```

#### Entity: Notification

```php
#[ORM\Entity(repositoryClass: NotificationRepository::class)]
#[ORM\Table(name: 'notification')]
class Notification
{
    #[ORM\Id]
    #[ORM\GeneratedValue]
    #[ORM\Column]
    private ?int $id = null;

    #[ORM\ManyToOne(targetEntity: Agent::class)]
    #[ORM\JoinColumn(nullable: false)]
    private ?Agent $agent = null;

    #[ORM\ManyToOne(targetEntity: Reservation::class)]
    #[ORM\JoinColumn(nullable: true)]
    private ?Reservation $reservation = null;

    #[ORM\Column(length: 50)]
    private string $type; // confirmation, rappel, modification, annulation, invitation

    #[ORM\Column(length: 255)]
    private string $sujet;

    #[ORM\Column(type: Types::TEXT)]
    private string $contenu;

    #[ORM\Column(length: 20)]
    private string $statut = 'pending'; // pending, sent, failed

    #[ORM\Column(type: Types::DATETIME_MUTABLE, nullable: true)]
    private ?\DateTimeInterface $sentAt = null;

    #[ORM\Column(type: Types::DATETIME_MUTABLE)]
    private ?\DateTimeInterface $createdAt = null;
}
```

### 2.3 Nouvelles Routes

```yaml
# Routes Créneaux (Sophie - Gestionnaire)
app_creneau_index:       GET    /campagnes/{campagne}/creneaux
app_creneau_new:         GET|POST /campagnes/{campagne}/creneaux/nouveau
app_creneau_edit:        GET|POST /campagnes/{campagne}/creneaux/{id}/modifier
app_creneau_delete:      POST   /campagnes/{campagne}/creneaux/{id}/supprimer
app_creneau_generate:    GET|POST /campagnes/{campagne}/creneaux/generer

# Routes Réservation Agent (Public - token)
app_booking_index:       GET    /reservation/{token}
app_booking_select:      POST   /reservation/{token}/choisir/{creneau}
app_booking_confirm:     POST   /reservation/{token}/confirmer
app_booking_cancel:      POST   /reservation/{token}/annuler
app_booking_recap:       GET    /reservation/{token}/recapitulatif

# Routes Manager (Authentifié)
app_manager_agents:      GET    /manager/campagne/{campagne}/agents
app_manager_position:    GET|POST /manager/campagne/{campagne}/positionner/{agent}
app_manager_modify:      GET|POST /manager/campagne/{campagne}/modifier/{reservation}
app_manager_cancel:      POST   /manager/campagne/{campagne}/annuler/{reservation}
app_manager_planning:    GET    /manager/campagne/{campagne}/planning

# Routes Coordinateur
app_coord_agents:        GET    /coordinateur/campagne/{campagne}/agents
app_coord_position:      GET|POST /coordinateur/campagne/{campagne}/positionner/{agent}
```

### 2.4 Services

```php
// src/Service/CreneauService.php
class CreneauService
{
    public function creer(Campagne $campagne, array $data): Creneau;
    public function genererPlage(Campagne $campagne, \DateTime $debut, \DateTime $fin, int $duree, int $capacite): array;
    public function modifier(Creneau $creneau, array $data): Creneau;
    public function supprimer(Creneau $creneau): void;
    public function getDisponibles(Campagne $campagne, ?Segment $segment = null): array;
    public function verrouillerAutomatique(): int; // Cron J-X
}

// src/Service/ReservationService.php
class ReservationService
{
    public function reserver(Agent $agent, Creneau $creneau, string $type, ?Utilisateur $par = null): Reservation;
    public function modifier(Reservation $reservation, Creneau $nouveauCreneau): Reservation;
    public function annuler(Reservation $reservation): void;
    public function getByAgent(Agent $agent, Campagne $campagne): ?Reservation;
    public function getByManager(Utilisateur $manager, Campagne $campagne): array;
}

// src/Service/NotificationService.php
class NotificationService
{
    public function envoyerConfirmation(Reservation $reservation): void;
    public function envoyerRappel(Reservation $reservation): void;
    public function envoyerModification(Reservation $reservation, Creneau $ancienCreneau): void;
    public function envoyerAnnulation(Reservation $reservation): void;
    public function envoyerInvitation(Agent $agent, Campagne $campagne): void;
    public function genererICS(Reservation $reservation): string;
}
```

---

## 3. Plan de Sprints V2

### Sprint 16 — Setup & Entités (1 semaine)

|ID|Tâche|US|Effort|
|---|---|---|---|
|T-1601|Créer entité `Creneau` + migration|-|2h|
|T-1602|Créer entité `Agent` + migration|-|2h|
|T-1603|Créer entité `Reservation` + migration|-|2h|
|T-1604|Créer entité `Notification` + migration|-|1h|
|T-1605|Créer repositories (Creneau, Agent, Reservation)|-|2h|
|T-1606|Créer `CreneauService` (squelette)|-|1h|
|T-1607|Créer `ReservationService` (squelette)|-|1h|
|T-1608|Créer `NotificationService` (squelette)|-|1h|
|T-1609|Fixtures : 50 agents, 3 services, 2 managers|-|2h|
|T-1610|CRUD Agent dans EasyAdmin|-|1h|

**Livrables** : 4 entités, 4 migrations, 3 services, fixtures

---

### Sprint 17 — Gestion Créneaux (EPIC-11 Core) (1 semaine)

|ID|Tâche|US|Effort|
|---|---|---|---|
|T-1701|`CreneauController` : index, new, edit, delete|US-1101, US-1104, US-1105|4h|
|T-1702|Templates créneaux (liste, formulaire)|-|3h|
|T-1703|`CreneauService::genererPlage()`|US-1101|2h|
|T-1704|Template génération automatique créneaux|US-1101|2h|
|T-1705|Widget taux de remplissage|US-1106|2h|
|T-1706|Logique verrouillage J-X|US-1107|2h|
|T-1707|Association créneaux ↔ segments|US-1108|2h|
|T-1708|Tests CreneauService|-|2h|

**Livrables** : CRUD complet créneaux, génération automatique, verrouillage

---

### Sprint 18 — Interface Réservation (EPIC-10 Core) (1.5 semaines)

|ID|Tâche|US|Effort|
|---|---|---|---|
|T-1801|`BookingController` : interface agent|US-1001|3h|
|T-1802|Template liste créneaux disponibles|US-1001|3h|
|T-1803|`ReservationService::reserver()` + unicité|US-1002|3h|
|T-1804|Template confirmation réservation|US-1002|2h|
|T-1805|Annulation/modification agent|US-1003|3h|
|T-1806|`ManagerController` : liste agents|US-1005|3h|
|T-1807|Template vue manager (agents + statuts)|US-1005|3h|
|T-1808|Positionnement par manager|US-1006|3h|
|T-1809|Modification/annulation par manager|US-1007|3h|
|T-1810|Tests ReservationService|-|3h|

**Livrables** : Interface agent complète, interface manager complète

---

### Sprint 19 — Notifications (EPIC-12) (1 semaine)

|ID|Tâche|US|Effort|
|---|---|---|---|
|T-1901|`NotificationService::genererICS()`|US-1201|3h|
|T-1902|Template email confirmation + ICS|US-1201|2h|
|T-1903|`NotificationService::envoyerConfirmation()`|US-1201|2h|
|T-1904|Email rappel J-2 (command cron)|US-1202|3h|
|T-1905|Email modification (ancien + nouveau)|US-1203|2h|
|T-1906|Email annulation + lien repositionnement|US-1204|2h|
|T-1907|Email invitation initiale|US-1205|2h|
|T-1908|Configuration SMTP + tests envoi|-|2h|
|T-1909|Tests NotificationService|-|2h|

**Livrables** : 5 types d'emails, génération ICS, commande rappel

---

### Sprint 20 — Compléments V1 (1 semaine)

|ID|Tâche|US|Effort|
|---|---|---|---|
|T-2001|Page récapitulatif agent|US-1004|2h|
|T-2002|Vue planning manager (répartition équipe)|US-1008|4h|
|T-2003|Alerte concentration >50%|US-1008|2h|
|T-2004|Interface coordinateur|US-1010|3h|
|T-2005|Authentification AD (fallback carte agent)|US-1011|4h|
|T-2006|Définition capacité IT (abaques)|US-1102, US-1103|3h|
|T-2007|Configuration verrouillage par campagne|US-1107|1h|
|T-2008|Filtrage créneaux par segment/site|US-1108|2h|

**Livrables** : Fonctionnalités complémentaires, auth AD

---

### Sprint 21 — Tests & Audit P6 (1 semaine)

|ID|Tâche|US|Effort|
|---|---|---|---|
|T-2101|Tests E2E parcours agent|-|3h|
|T-2102|Tests E2E parcours manager|-|3h|
|T-2103|Tests E2E notifications|-|2h|
|T-2104|Audit P6.1-P6.6 (Qualify)|-|4h|
|T-2105|Corrections findings P6|-|4h|
|T-2106|Documentation utilisateur V2|-|3h|
|T-2107|🏷 TAG v2.0.0|-|1h|

**Livrables** : Tests complets, audit V2 READY, tag v2.0.0

---

## 4. User Stories Détaillées par Sprint

### 4.1 Sprint 17 — EPIC-11 (4 US)

#### US-1101 : Créer des créneaux pour une campagne

```gherkin
Feature: Création de créneaux
  
  Scenario: Création manuelle d'un créneau
    Given Sophie est sur la page créneaux d'une campagne
    When elle clique sur "+ Nouveau créneau"
    And remplit date, heure début, heure fin, capacité, lieu
    And valide
    Then le créneau est créé
    And il apparaît dans la liste

  Scenario: Génération automatique de créneaux
    Given Sophie configure une génération automatique
    When elle définit : du 01/02 au 15/02, 9h-12h + 14h-17h, slots 30min, capacité 2
    And valide
    Then X créneaux sont générés automatiquement
```

#### US-1104 : Modifier un créneau

```gherkin
Feature: Modification de créneau

  Scenario: Modification sans réservation
    Given un créneau sans réservation
    When Sophie modifie la date/heure
    Then les modifications sont enregistrées

  Scenario: Modification avec réservations
    Given un créneau avec 3 réservations
    When Sophie modifie la date
    Then elle confirme que les agents seront notifiés
    And les 3 agents reçoivent un email de modification
```

#### US-1105 : Supprimer un créneau

```gherkin
Feature: Suppression de créneau

  Scenario: Suppression créneau vide
    Given un créneau sans réservation
    When Sophie clique sur Supprimer
    Then le créneau est supprimé immédiatement

  Scenario: Suppression créneau avec réservations
    Given un créneau avec 2 réservations
    When Sophie clique sur Supprimer
    Then un warning s'affiche "2 agents seront notifiés"
    And après confirmation, les agents reçoivent email d'annulation
```

#### US-1106 : Voir le taux de remplissage

```gherkin
Feature: Taux de remplissage

  Scenario: Affichage visuel
    Given des créneaux avec différents taux
    When Sophie consulte la liste
    Then elle voit "X/Y places (Z%)" avec code couleur
    And Vert <50%, Orange 50-90%, Rouge >90%
```

---

### 4.2 Sprint 18 — EPIC-10 (6 US)

#### US-1001 : Voir les créneaux disponibles (Agent)

```gherkin
Feature: Liste créneaux agent

  Scenario: Accès via lien email
    Given un agent reçoit un email d'invitation
    When il clique sur le lien
    And s'authentifie
    Then il voit les créneaux disponibles pour sa campagne

  Scenario: Créneaux complets grisés
    Given un créneau est complet
    When l'agent consulte les créneaux
    Then ce créneau est grisé avec "Complet"
    And il n'est pas cliquable
```

#### US-1002 : Se positionner sur un créneau

```gherkin
Feature: Réservation agent

  Scenario: Confirmation réservation
    Given l'agent sélectionne un créneau
    When il confirme
    Then la réservation est enregistrée
    And il reçoit email confirmation + ICS
    And le compteur de places décrémente

  Scenario: Unicité par campagne (RG-121)
    Given l'agent a déjà une réservation
    When il tente de réserver un autre créneau
    Then message "Vous avez déjà un créneau. Annulez d'abord."
```

#### US-1003 : Annuler/modifier son créneau

```gherkin
Feature: Modification par agent

  Scenario: Annulation
    Given l'agent a une réservation
    When il clique Annuler et confirme
    Then la réservation est annulée
    And la place est libérée
    And il reçoit email d'annulation

  Scenario: Verrouillage J-2 (RG-123)
    Given le créneau est dans 1 jour
    When l'agent tente de modifier
    Then message "Créneau verrouillé. Contactez votre manager."
```

#### US-1005 : Voir la liste de mes agents (Manager)

```gherkin
Feature: Vue manager

  Scenario: Liste équipe
    Given le manager se connecte
    When il consulte une campagne
    Then il voit ses agents avec statut ✅ Positionné / ❌ Non positionné
    And compteur "6/10 agents positionnés (60%)"
```

#### US-1006 : Positionner un agent

```gherkin
Feature: Positionnement par manager

  Scenario: Positionner un agent
    Given le manager voit un agent non positionné
    When il clique Positionner
    And sélectionne un créneau
    Then l'agent est positionné
    And l'agent reçoit notification

  Scenario: Traçabilité (RG-125)
    Given le manager positionne un agent
    Then la réservation indique "Positionné par: [Manager]"
```

#### US-1007 : Modifier/annuler le créneau d'un agent

```gherkin
Feature: Modification par manager

  Scenario: Modification
    Given un agent de mon équipe est positionné
    When je clique Modifier
    Then je peux choisir un autre créneau
    And l'agent reçoit notification (RG-126)

  Scenario: Remplacement rapide
    Given un agent est absent
    When je l'annule
    Then je peux immédiatement positionner un remplaçant
```

---

### 4.3 Sprint 19 — EPIC-12 (5 US)

#### US-1201 : Email confirmation avec ICS (RG-140)

```gherkin
Feature: Confirmation ICS

  Scenario: Contenu email
    Given une réservation est confirmée
    Then l'agent reçoit email avec :
      | Objet: "[OpsTracker] Votre rendez-vous du [date] est confirmé" |
      | Corps: Date, heure, lieu, description |
      | PJ: RDV.ics |

  Scenario: Fichier ICS
    Given l'agent ouvre le fichier ICS
    Then Outlook propose d'ajouter l'événement
    With rappel J-1
```

#### US-1202 : Email rappel J-2 (RG-141)

```gherkin
Feature: Rappel automatique

  Scenario: Envoi rappel
    Given une réservation dans 2 jours
    When le cron s'exécute
    Then l'agent reçoit "Rappel : votre rendez-vous dans 2 jours"
```

#### US-1203 : Email modification (RG-142)

```gherkin
Feature: Notification modification

  Scenario: Contenu modification
    Given une réservation est modifiée
    Then l'agent reçoit email avec :
      | Ancien créneau: [date1] [heure1] |
      | Nouveau créneau: [date2] [heure2] |
      | PJ: Nouvel ICS |
```

#### US-1204 : Email annulation (RG-143)

```gherkin
Feature: Notification annulation

  Scenario: Contenu annulation
    Given une réservation est annulée
    Then l'agent reçoit email avec :
      | Sujet: "Votre créneau a été annulé" |
      | Lien: vers interface de repositionnement |
```

#### US-1205 : Invitation initiale (RG-144)

```gherkin
Feature: Email invitation

  Scenario: Mode agent
    Given campagne en mode inscription "Agent"
    When les invitations sont envoyées
    Then chaque agent reçoit lien vers interface réservation

  Scenario: Mode manager
    Given campagne en mode inscription "Manager"
    When les invitations sont envoyées
    Then chaque manager reçoit lien vers interface positionnement
```

---

## 5. Règles Métier V2

|Code|Règle|US Liées|
|---|---|---|
|**RG-120**|Un agent ne voit que les créneaux de son segment/site|US-1001|
|**RG-121**|Un agent = un seul créneau par campagne|US-1002, US-1006|
|**RG-122**|Confirmation automatique = email + ICS|US-1002|
|**RG-123**|Verrouillage J-X (défaut: 2)|US-1003, US-1107|
|**RG-124**|Manager ne voit que les agents de son service|US-1005|
|**RG-125**|Traçabilité : qui a positionné|US-1006, US-1010|
|**RG-126**|Notification agent si positionné par tiers|US-1007|
|**RG-127**|Alerte si >50% équipe même jour|US-1008|
|**RG-130**|Création créneaux : manuelle ou génération|US-1101|
|**RG-133**|Modification créneau = notification agents|US-1104|
|**RG-134**|Suppression créneau = confirmation si réservations|US-1105|
|**RG-135**|Association créneau ↔ segment optionnelle|US-1108|
|**RG-140**|Email confirmation contient ICS obligatoire|US-1201|
|**RG-141**|Email rappel automatique J-X|US-1202|
|**RG-142**|Email modification = ancien + nouveau + ICS|US-1203|
|**RG-143**|Email annulation = lien repositionnement|US-1204|
|**RG-144**|Invitation selon mode (agent/manager)|US-1205|

---

## 6. Configuration Technique

### 6.1 SMTP (Notifications)

```yaml
# .env
MAILER_DSN=smtp://smtp.cpam92.local:25

# config/packages/mailer.yaml
framework:
    mailer:
        dsn: '%env(MAILER_DSN)%'
        envelope:
            sender: 'opstracker@cpam92.local'
```

### 6.2 Commande Cron (Rappels)

```php
// src/Command/SendReminderCommand.php
#[AsCommand(name: 'app:send-reminders', description: 'Envoie les rappels J-2')]
class SendReminderCommand extends Command
{
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        $count = $this->notificationService->envoyerRappelsJour(2);
        $output->writeln("$count rappels envoyés.");
        return Command::SUCCESS;
    }
}
```

```bash
# Crontab (tous les jours à 8h)
0 8 * * * cd /var/www/opstracker && php bin/console app:send-reminders
```

### 6.3 Génération ICS

```php
// src/Service/IcsGenerator.php
class IcsGenerator
{
    public function generate(Reservation $reservation): string
    {
        $creneau = $reservation->getCreneau();
        $agent = $reservation->getAgent();
        $campagne = $reservation->getCampagne();
        
        $dtStart = $creneau->getDate()->format('Ymd') . 'T' . $creneau->getHeureDebut()->format('His');
        $dtEnd = $creneau->getDate()->format('Ymd') . 'T' . $creneau->getHeureFin()->format('His');
        
        return <<<ICS
BEGIN:VCALENDAR
VERSION:2.0
PRODID:-//OpsTracker//Organisation92//FR
BEGIN:VEVENT
UID:{$reservation->getId()}@opstracker.cpam92.local
DTSTART:$dtStart
DTEND:$dtEnd
SUMMARY:[{$campagne->getNom()}] Intervention IT
LOCATION:{$creneau->getLieu()}
DESCRIPTION:Intervention prévue pour {$agent->getPrenom()} {$agent->getNom()}
BEGIN:VALARM
TRIGGER:-P1D
ACTION:DISPLAY
DESCRIPTION:Rappel intervention IT demain
END:VALARM
END:VEVENT
END:VCALENDAR
ICS;
    }
}
```

---

## 7. Critères d'Acceptance V2

### 7.1 Definition of Done (Sprint)

- [ ] Code implémenté et fonctionnel
- [ ] Tests unitaires passants (>80% couverture)
- [ ] Scénarios BDD validés manuellement
- [ ] Code reviewé (auto-review Claude Code)
- [ ] Pas de régression sur V1
- [ ] Commit avec message conventionnel

### 7.2 Definition of Done (V2)

- [ ] 26 US implémentées
- [ ] Audit P6 passé (score ≥95%)
- [ ] 0 findings bloquants
- [ ] Tests E2E parcours agent/manager
- [ ] Documentation utilisateur V2
- [ ] Performance validée (<500ms sur 1000 créneaux)
- [ ] 🏷 TAG v2.0.0

---

## 8. Risques & Mitigations

|Risque|Probabilité|Impact|Mitigation|
|---|---|---|---|
|SMTP non configuré|Moyenne|Fort|Configurer dès Sprint 19, fallback log|
|Complexité auth AD|Moyenne|Moyen|Prévoir fallback email/password|
|Volume agents (>1000)|Faible|Moyen|Pagination + index optimisés|
|ICS incompatible Outlook|Faible|Fort|Tester sur Outlook 365 dès Sprint 19|
|Conflits réservation (race condition)|Faible|Fort|Contrainte unique DB + lock optimiste|

---

## 9. Prochaines Étapes

1. ✅ Documentation V2 complète
2. 🔜 Sprint 16 : Setup entités
3. 🔜 Sprint 17-18 : Core EPIC-10/11
4. 🔜 Sprint 19 : Notifications
5. 🔜 Sprint 20-21 : Compléments + Audit P6
6. 🔜 TAG v2.0.0

---

_Document généré le 2025-01-24 — Framework BA-AI P5_