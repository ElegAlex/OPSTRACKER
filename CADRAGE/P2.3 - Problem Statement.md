...
|POC TAM → Validation|✅|Preuve de concept existante, concepts validés terrain|
...
> _"Les gestionnaires d'opérations IT des CPAM passent plus de temps à consolider des fichiers Excel qu'à piloter leurs opérations, pendant que les techniciens terrain jonglent entre sources dispersées — faute d'une source unique de vérité qui soit simple, souveraine et conforme RGAA. Le POC migration TAM IDF a prouvé que le concept fonctionne. Il faut maintenant le génériciser."_

### 📐 Définition Structurée

|Dimension|Définition|
|---|---|
|**POUR**|Les gestionnaires d'opérations IT (Sophie) et techniciens terrain (Karim) des CPAM|
...
|---|---|---|
|**Adoption terrain (Karim)**|Risque **réel mais gérable** — 50% des techniciens trouvent leurs outils difficiles (Microsoft). **Mitigation** : Voice of the Technician, UX laptop-first, capacité offline, POC TAM a déjà validé l'appétence|Si adoption <70% au pilote → itération UX rapide (vibe coding = cycles courts)|
...
|**RGAA sous-estimé**|Risque **réel** — Compte ameli à 46.51%. **Mitigation** : Intégrer RGAA dès conception, pas en retrofit. Outils automatiques (axe-core) + revue manuelle critères critiques|Cible 75% AA au MVP, amélioration continue|
...
- [x] **Le problème est validé par un POC fonctionnel** (Migration TAM IDF — concepts prouvés)
...
|Axe|Évaluation|
|---|---|
|**Désirabilité**|✅ Problème réel, documenté, POC validé, demande multi-CPAM (37, 75, 77, 93)|
...
1. **Le POC TAM IDF existe** — Les concepts sont validés terrain, pas besoin de re-tester
