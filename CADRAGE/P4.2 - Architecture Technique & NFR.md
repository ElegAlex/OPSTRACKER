...
```mermaid
graph TB
    subgraph Organisation["🏢 RÉSEAU Organisation (Self-hosted)"]
        ...
        Users -->|"HTTPS<br/>Cert. interne"| nginx
        nginx -->|"FastCGI"| php
        php -->|"SQL"| postgres
...
|**Statut**|✅ Accepté|
|**Date**|2026-01-19|
|**Contexte**|P0 stipule "V1 : Comptes locaux / V2 : SSO Organisation". Self-hosted, pas d'exposition externe.|
...
|**Temps chargement Dashboard**|< 1s sur 100 000 opérations|Test de charge|Scalabilité Organisation multi-campagnes|
...
|**Chiffrement transit**|TLS 1.3 obligatoire|Nginx config, certificat interne|
...
**Note** : Hébergement self-hosted = pas de transfert vers cloud US (CLOUD Act non applicable).
...
|**Backup**|pg_dump quotidien 02h00|Cron + stockage local Organisation|
...
|**Décision Sponsor**|✅ **ACCEPTÉ** — Le sponsor accepte le risque car : (1) réseau interne isolé, (2) utilisateurs de confiance (techniciens IT), (3) besoin métier réel (outils de migration).|
...
│   ├── nginx/
│   │   ├── nginx.conf
│   │   └── certs/                      # Certificat SSL Organisation
│   └── php/
│       └── Dockerfile
├── docker-compose.yml
...
## 8. Plan de Déploiement (Self-hosted Organisation)
...
```mermaid
graph TB
    subgraph "Serveur Organisation (Docker Host)"
    ...
    user[Utilisateurs Organisation] -->|HTTPS<br/>Certificat interne| nginx
...
services:
  nginx:
    ...
    volumes:
      ...
      - ./docker/nginx/certs:/etc/nginx/certs:ro  # Certificat Organisation
...
- [ ] Docker + Docker Compose installés sur serveur Organisation
- [ ] Certificat SSL interne déposé dans `docker/nginx/certs/`
...
|2|**Certificat SSL**|Interne ou Let's Encrypt ?|✅ Certificat interne|Config Nginx|
...
_Les 5% d'incertitude portent sur : (1) disponibilité effective du Bundle interne Symfony, (2) validation performance JSONB sur volumes réels (spike P4.3)._
