...
```mermaid
graph TB
    subgraph CPAM["🏢 RÉSEAU CPAM 92 (Self-hosted)"]
        ...
        subgraph Docker["🐳 Docker Host (Serveur)"]
            subgraph Containers["docker-compose.yml"]
                nginx["🌐 Nginx<br/>Reverse Proxy :443"]
                php["⚙️ PHP-FPM 8.3<br/>───────────<br/>• Symfony 7.4 LTS<br/>• Twig + Turbo/Stimulus<br/>• EasyAdmin 4<br/>• Symfony Workflow<br/>• Symfony Messenger"]
                postgres[("🗄️ PostgreSQL 17<br/>(JSONB)")]
                redis[("⚡ Redis 7<br/>Sessions / Cache")]
            end
        end
        
        Users -->|"HTTPS<br/>Cert. interne CPAM"| nginx
        nginx -->|"FastCGI"| php
        php -->|"SQL"| postgres
...
graph TB
    subgraph Presentation["🎨 COUCHE PRÉSENTATION"]
        subgraph Twig["Twig + Hotwire"]
            dashboard["📊 Dashboard<br/>(Sophie)"]
            planning["📅 Planning<br/>(Sophie)"]
            terrain["🔧 Terrain<br/>(Karim)"]
            admin["⚙️ Admin<br/>(EasyAdmin 4)"]
        end
    end
    ...
    subgraph Infrastructure["🏗️ COUCHE INFRASTRUCTURE"]
...
sequenceDiagram
    ...
    Note over K,App: Temps réel IMMÉDIAT (utilisateur qui modifie)
    K->>App: POST /operation/123/statut (RDV → Réalisé)
    App->>DB: UPDATE operation SET statut = 'realise'
    App-->>K: 200 OK + Turbo Stream (refresh carte instantané)
    ...
    loop Toutes les 30 secondes
        ...
        App-->>S: HTML partiel (compteurs mis à jour)
    end
```
...
**Note sur Symfony 8.0** : Disponible mais support court (jusqu'à juillet 2026). Pour un projet CPAM en production, **Symfony 7.4 LTS** est le choix recommandé pour la stabilité long terme.
...
|**Statut**|✅ Accepté|
|**Date**|2026-01-19|
|**Contexte**|P0 stipule "V1 : Comptes locaux / V2 : SSO CNAM". Self-hosted, pas d'exposition externe.|
...
<turbo-frame id="dashboard-stats" data-controller="refresh" data-refresh-interval-value="30000">
...
|**Statut**|✅ Accepté|
|**Date**|2026-01-19|
|**Contexte**|Import massif de cibles : jusqu'à 100 000 lignes pour les grosses campagnes multi-CPAM|
...
|**Temps chargement Dashboard**|< 1s sur 100 000 opérations|Test de charge|Scalabilité CPAM multi-campagnes|
...
|**Chiffrement transit**|TLS 1.3 obligatoire|Nginx config, certificat interne CPAM|
...
**Note** : Hébergement self-hosted CPAM = pas de transfert vers cloud US (CLOUD Act non applicable).
...
|**Backup**|pg_dump quotidien 02h00|Cron + stockage local CPAM|
...
erDiagram
    ...
    Campagne ||--o{ Segment : "divisée en"
    Campagne ||--o{ Prerequis : "nécessite"
    ...
    TypeOperation ||--o{ Campagne : "définit"
    TypeOperation {
        int id PK
        string name
        string icon
        string color
        jsonb fields_schema "définition champs custom"
    ...
    ChecklistTemplate {
        ...
        string name
        int version
        jsonb structure "phases et étapes"
        boolean is_active
        datetime created_at
    }
    ...
    ChecklistInstance {
        ...
        jsonb snapshot "structure figée"
        jsonb progress "état des coches"
        datetime started_at
...
|4|**Bundle AM non disponible**|Moyen|Moyen|Architecture standalone, intégration ultérieure possible (ADR-002)|
...
|**Décision Sponsor**|✅ **ACCEPTÉ** — Le sponsor accepte le risque car : (1) réseau interne CPAM isolé, (2) utilisateurs de confiance (techniciens IT), (3) besoin métier réel (outils de migration).|
...
## 8. Plan de Déploiement (Self-hosted CPAM 92)
...
```mermaid
graph TB
    subgraph "Serveur CPAM 92 (Docker Host)"
    ...
    user[Utilisateurs CPAM] -->|HTTPS<br/>Certificat interne| nginx
...
services:
  nginx:
    ...
    volumes:
      ...
      - ./docker/nginx/certs:/etc/nginx/certs:ro  # Certificat CPAM
...
- [ ] Docker + Docker Compose installés sur serveur CPAM 92
- [ ] Certificat SSL interne CPAM déposé dans `docker/nginx/certs/`
...
- [ ] Backup existant CPAM configuré pour volumes Docker
...
|2|**Certificat SSL**|Interne ou Let's Encrypt ?|✅ Certificat interne CPAM|Config Nginx|
...
|5|**Backup**|Procédure existante ?|✅ Backup interne quotidien CPAM|Pas de config additionnelle|
