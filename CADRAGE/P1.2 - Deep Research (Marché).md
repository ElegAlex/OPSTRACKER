...
**Contexte cible** : Organisation (secteur public santé français), self-hosted, Symfony obligatoire, RGAA 4.1.
...
**Constat** : Les solutions spécialisées sont **très coûteuses**, **complexes à déployer**, et **orientées grandes entreprises privées**. Aucune n'est adaptée au contexte Organisation (budget contraint, self-hosted, accessibilité RGAA).
...
**Constat** : Les outils généralistes sont **flexibles mais cloud-only**, ce qui exclut le self-hosted. Aucun ne propose de **checklists terrain structurées par phases** avec protection des checklists "in progress". L'**accessibilité RGAA** n'est garantie sur aucun.
...
3. **Self-hosted souverain**
    ...
    - Données restent sur l'infra Organisation
...
4. **Checklists terrain protégées**
    ...
    - Répond au pain point POC initial identifié en P1.1
...
|**POC existant validé**|L'application pilote a prouvé le concept, demande multi-organisations confirmée|
