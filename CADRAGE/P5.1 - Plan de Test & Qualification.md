...
```mermaid
flowchart TB
    subgraph NIVEAU1["📋 Niveau 1 : Tests Unitaires"]
        U1["Services métier"]
        U2["Entités & Validations"]
        U3["Voters & Security"]
    end
    
    subgraph NIVEAU2["🔗 Niveau 2 : Tests d'Intégration"]
        I1["Repositories + PostgreSQL"]
        I2["Workflows Symfony"]
        I3["Import CSV + JSONB"]
    end
    
    subgraph NIVEAU3["🌐 Niveau 3 : Tests E2E"]
        E1["Parcours Sophie"]
        E2["Parcours Karim"]
        E3["Parcours Agent/Manager"]
    end
    
    subgraph NIVEAU4["♿ Niveau 4 : Tests Spécialisés"]
        A1["Accessibilité RGAA"]
...
|**staging**|PostgreSQL Organisation|Données anonymisées|UAT, performance|
|**prod**|PostgreSQL Organisation|Réelles|Post-déploiement smoke|
...
|ID|Scénario|Préconditions|Actions|Résultat Attendu|RG|
|---|---|---|---|---|---|
|TC-703-01|Liaison nominale|Doc existant|Associer à campagne "Migration Pilote Alpha"|Doc visible dans contexte campagne|RG-051|
...
gantt
    ...
    Tests intégration (CI)         :active, 2026-01-27, 83d
